...
async function runTraderDoctor(targetDir) {
  ...
  // Check 9: Cron jobs configured (3 new OpenClaw checks added)
  results.checks.push(await checkTradingCron(targetDir));
  
  // Check 10: OpenClaw gateway installed
  results.checks.push(await checkOpenClawInstalled());
  
  // Check 11: OpenClaw gateway running
  results.checks.push(await checkOpenClawRunning());
  
  // Check 12: OpenClaw API reachable
  results.checks.push(await checkOpenClawApi());
...
async function checkOpenClawInstalled() {
  try {
    ...
    return { name: 'OpenClaw Installed', status: 'ok', message: 'Found in PATH' };
  } catch {
    return { 
      name: 'OpenClaw Installed', 
      status: 'warn', 
      message: 'Not found - bot-runner will run without OpenClaw integration' 
    };
  }
}
...
async function checkOpenClawRunning() {
  try {
    execSync('systemctl is-active --quiet openclaw-gateway', { stdio: 'ignore' });
    return { name: 'OpenClaw Service', status: 'ok', message: 'Active' };
  } catch {
    try {
      execSync('systemctl is-enabled --quiet openclaw-gateway', { stdio: 'ignore' });
      return { 
      ...
      return { 
        name: 'OpenClaw Service', 
        status: 'warn', 
        message: 'Not installed (install for full integration)' 
      };
    }
  }
}
...
async function checkOpenClawApi() {
  const gatewayUrl = process.env.OPENCLAW_GATEWAY_URL || 'http://localhost:8081';
  try {
    execSync(`curl -sf ${gatewayUrl}/healthz -o /dev/null`, { timeout: 3000 });
    return { name: 'OpenClaw API', status: 'ok', message: `${gatewayUrl} reachable` };
  } catch {
    return { 
      name: 'OpenClaw API', 
      status: 'warn', 
      message: `${gatewayUrl} not reachable` 
    };
  }
}
