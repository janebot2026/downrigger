 ...
 * 2. Install binaries / validate paths
 * 3. Write config TOML + env
 * 4. Create systemd unit
 * 5. Set cron jobs
 * 6. Run doctor checks and print green lights report
 */
async function installTraderCommand(options) {
  ...
  // Step 2: Install binaries / validate paths
  const step2 = ora('Step 2/6: Validating binaries...').start();
  const binaryChecks = {
  ...
  try {
    ...
    if (!binaryChecks.botRunner.found || !binaryChecks.clawTrader.found) {
      step2.warn('Some binaries not found');
      ...
      step2.succeed(`Binaries validated: bot-runner, claw-trader`);
    }
  } catch (err) {
  ...
  const step3 = ora('Step 3/6: Writing configuration...').start();
  try {
...
`;
  ...
  const step4 = ora('Step 4/6: Creating systemd service...').start();
  try {
...
`;
  ...
  const step5 = ora('Step 5/6: Setting up cron jobs...').start();
  try {
  ...
  const step6 = ora('Step 6/6: Running health checks...').start();
  try {
