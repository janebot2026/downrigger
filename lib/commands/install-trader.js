 ...
 * 5. Setup SQLite tracking databases
 * 6. Create systemd units
 * 7. Set cron jobs
 * 8. Run doctor checks and print green lights report
 */
async function installTraderCommand(options) {
  ...
  const step1 = ora('Step 1/8: Creating workspace structure...').start();
  try {
...
\n  }
  ...
  const step2 = ora('Step 2/8: Configuring OpenClaw (LLM + Telegram)...').start();
  ...
  const step3 = ora('Step 3/8: Validating binaries...').start();
  const binaryChecks = {
  ...
  try {
    ...
    if (missingBinaries.length > 0) {
      step3.warn(`Some binaries not found: ${missingBinaries.join(', ')}`);
      ...
      step3.succeed(`Binaries validated: bot-runner, claw-trader${openclawStatus}`);
    }
  ...
  // Step 4: Write config TOML + env
  const step4 = ora('Step 4/8: Writing configuration...').start();
  try {
...
`;
    ...
    step4.succeed('Configuration written');
  } catch (err) {
    step4.fail(`Failed: ${err.message}`);
    throw err;
  }
  ...
  const sqliteSpinner = ora('Step 5/8: Setting up SQLite tracking databases...').start();
  try {
  ...
  const step5 = ora('Step 6/8: Creating systemd service...').start();
  try {
...
`;
    ...
    step5.succeed(`Systemd service created (${serviceName} in workspace)`);
  ...
  const step6 = ora('Step 7/8: Setting up cron jobs...').start();
  try {
    const result = await setupTraderCron(targetDir, { agentName, force });
    step6.succeed(`Cron jobs configured: ${result.created.length} new`);
  ...
  // Step 7: Run doctor checks
  const step7 = ora('Step 8/8: Running health checks...').start();
  try {
    const results = await runTraderHealthChecks(targetDir, binaryChecks);
    step7.succeed('Health checks complete');
    
    // Print green lights report
    printGreenLightsReport(results);
  } catch (err) {
    step7.fail(`Failed: ${err.message}`);
  }
