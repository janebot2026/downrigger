...
async function checkCoreFiles(targetDir) {
  ...
  for (const file of requiredFiles) {
    ...
    if (!await fs.pathExists(fullPath)) {
      issues.push({
        message: `Missing core file: ${file} (run: downrigger template ${file} -o ${path.join(targetDir, file)})`,
        fixable: false
      });
    }
  }
...
async function checkQmd(targetDir) {
  ...
  if (!await fs.pathExists(qmdDir)) {
    issues.push({
      message: `QMD not installed (missing ${qmdDir}). Run: downrigger install qmd`,
      fixable: true
    });
    return { ok: issues.length === 0, issues };
  }
  ...
  try {
    ...
    issues.push({
      message: `QMD not installed or not functional (run: downrigger install qmd). Cause: ${err.message}`,
      fixable: true
    });
  }
...
async function checkScripts(targetDir) {
  ...
  for (const script of requiredScripts) {
    ...
    if (!await fs.pathExists(fullPath)) {
      issues.push({
        message: `Missing script: ${script} (run: downrigger install scripts -d ${targetDir})`,
        fixable: false
      });
    }
  }
...
async function checkOpenClawCron() {
  ...
  if (!exists) {
    issues.push({
      message: `OpenClaw cron jobs.json not found: ${jobsPath} (run: downrigger install cron)`,
      fixable: false
    });
  }
  return { ok: issues.length === 0, issues };
