...
async function promptOpenClawConfig() {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      when: (answers) => answers.enableTelegram,
      validate: (input) => {
        if (!input) return 'User ID is required for security';
        if (!input.match(/^\d+$/)) return 'User ID should be numbers only';
        return true;
      }
    }
  ]);
...
function renderOpenClawConfig(config) {
  ...
  return {
    ...
    // Channels configuration - FIXED to match working schema
    channels: {
      telegram: {
        ...
        streamMode: 'partial',
        allowFrom: config.telegram.userId ? [config.telegram.userId] : []
      }
    },
...
TELEGRAM_USER_ID="${config.channels.telegram.allowFrom[0] || ''}"
